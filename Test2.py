...
def find_nearest_color(target_hex, color_dict, threshold=20):  # ~0.1% in 8-bit color scale
    """Find the closest matching color in the dictionary within a given threshold."""
    target_rgb = hex_to_rgb(target_hex)
    for name, hex_color in color_dict.items():
        if color_distance(target_rgb, hex_to_rgb(hex_color)) <= threshold:
            return name, hex_color
    return None, None  # No close match

def modify_svg(input_file, output_file, color_dict):
    ...
    # Match paths with a fill color (including minor variations)
    pattern = re.compile(r'(<path[^>]*fill=["\'](#?[0-9a-fA-F]{6})["\'][^>]*>)')

    def replace_function(match):
        path_tag, detected_hex = match.groups()
        name, matched_hex = find_nearest_color(detected_hex, color_dict)
        
        if not name:  # Skip if no close match found
            return path_tag

        light_color = lighten_color(matched_hex)
        modified_tag = re.sub(r'/>$', '', path_tag.strip())  # Remove self-closing

        return (f'{modified_tag} onmouseover="this.style.fill=\'{light_color}\'; this.style.cursor=\'pointer\';" '
                f'onmouseout="this.style.fill=\'{matched_hex}\';" ' 
                f'onclick="alert(\'{name}\');"/>')

    svg_content = pattern.sub(replace_function, svg_content)
